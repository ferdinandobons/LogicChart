...
2. Use `logicchart doctor` when install, dependency, or parser capability issues are
   unclear.
3. For optional LLM setup, start with `logicchart llm providers`, `logicchart llm setup`,
   `logicchart llm show`, and `logicchart enrich`; use `logicchart llm setup --help` or
   `logicchart enrich --help` before advanced flags, prefer `--api-key-stdin` in scripts,
   review the local preview output before `--send`, and never print or commit keys.
